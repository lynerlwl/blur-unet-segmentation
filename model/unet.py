...
class spaG(nn.Module):
    def __init__(self, in_channels, out_channels):
        super().__init__()
        self.gconv1 = SAGEConv(in_channels, out_channels, aggr='sum') # mean, max, sum # median # 
    
    def forward(self, x):
        batch_size, channels, height, width = x.shape
        graph = feature_map_to_graph(x, conn=8, weight='euclidean')# euclidean | proposed
...
class noG(nn.Module):
    ...
    def forward(self, x):
        batch_size, channels, height, width = x.shape
        graph = feature_map_to_graph(x, conn=8)
        gresized =  graph.x.view(batch_size, channels, height, width) # Reverse the flattening of node features
        return gresized
...
class UNet(nn.Module):
    def __init__(self, n_channels, n_classes, graph=True, graph_message_passing=True):
        bilinear=False
        ...
        if graph: 
            self.gconv = spaG(1024, 1024) if graph_message_passing else noG(1024, 1024) 
        else:
            self.gconv = ''
