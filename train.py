...
log_name = '10-wood100-ori-gf-proposed'
logging.basicConfig(filename=f"log/{log_name}.log", level=logging.INFO, format='%(asctime)s - %(message)s', datefmt='%d-%b-%y %H:%M:%S')
path = "data/wood_pores/train"
# path = "data/spray/train"
dir_img = Path(f'{path}/img-gf0.5/')#
dir_mask = Path(f'{path}/msk-gf0.5/')
dir_checkpoint = Path('checkpoint/')
load = 'checkpoint/epoch3.pth' # False #     
val_percent = 0.1
batch_size = 1
num_epochs = 3
learning_rate = 1e-4 #1e-3 is too low
...
if __name__ == '__main__':
    ...
    model = UNet(n_channels=3, n_classes=2, bilinear=False, graph=True, graph_message_passing=True)
    # model = UNet3(n_channels=3, n_classes=2, bilinear=False, graph=False, graph_message_passing=True)
    ...
    optimizer = RMSprop(model.parameters(), lr=learning_rate, weight_decay=1e-6, momentum=0.9)
